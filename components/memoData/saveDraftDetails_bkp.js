...
class SaveDraftData {
    ...
    async saveDraftInvData(dataObj) {
        ...
        try {
            ...
            console.log('igstAmt', igstAmt, 'cgstAmt', cgstAmt, 'sgstAmt', sgstAmt, 'baseAmt', baseAmt, 'additionalAmt', additionalAmt, 'tdAmt', tdAmt)
            totalAmt = (igstAmt + cgstAmt + sgstAmt + baseAmt + additionalAmt - tdAmt)
        ...
        try {
            ...
            _.forEach(baseTransactions, (fl, key) => {
                ...
                var lrFound = allBaseTransData.filter(function (l) {
                    return (l.baseTransactionNumber == fl.baseTransactionNumber && l.glNumber == fl.glNumber); // && l.base_transaction_date == fl.base_transaction_date && l.baseTransactionType == fl.baseTransactionType);
                });
        ...
        try {
            ...
            if (lrList.length > 0) {
                ...
                for (var j = 0; j < lrList.length; j++) {
                    ...
                    if (d1.length) {
                        sumAdditional += await commonFuncs.converToNumber(lrList[j].amount)
                    } else
                        sumBase += await commonFuncs.converToNumber(lrList[j].amount)
                }
            }
            var validationMsg = '';
            ...
            console.log('sumAdditional', sumAdditional, 'invAddAmt', addAmt, 'sumBase', sumBase, 'invBaseAmt', baseAmt)

            if (addAmt.trucateNum() != sumAdditional.trucateNum()) {
                validationMsg = '-GL additional LR amount is mismatched.';
            }
            if (baseAmt.trucateNum() != sumBase.trucateNum()) {
                validationMsg += '-GL base LR amount is mismatched.';
            }
    ...
    async checkDuplicateLRs(lrData) {
        ...
        for (var i = 0; i < Object.keys(dupLRGroup).length; i++) {
            ...
            console.log('-----Object.values(dupLRGroup)', Object.values(dupGlGroup)[i])
            for (var j = 0; j < Object.keys(dupGlGroup).length; j++) {

                /*  var dupBDateGroup = _(Object.values(dupGlGroup)[j]).groupBy("baseTransactionDate").pickBy(x => x.length > 1).value();

                console.log(' Object.values(dupBDateGroup)', Object.values(dupBDateGroup)[j])
                for (var k = 0; k < Object.keys(dupBDateGroup).length; k++) {

                    var dupBTypeGroup = _(Object.values(dupBDateGroup)[k]).groupBy("baseTransactionType").pickBy(x => x.length > 1).value();

                    console.log(' Object.values(dupBTypeGroup)', Object.values(dupBTypeGroup)[k])
*/
                if (Object.keys(dupGlGroup).length > 0) { // multiple type date found
                    valMsg += '-Duplicate LRs(' + Object.keys(dupLRGroup)[i] + '/' + Object.keys(dupGlGroup)[j] + ') in request.';
                }
...
const saveDraft = async (req, res) => {
    ...
    try {
        ...
        if (valMsg != '') {
            ...
            if (reqObj.baseTransactions == undefined || reqObj.files == undefined)
            ...
            if (valMsg != '') {
                ...
                console.log('Bill Id to be updated ', reqObj.draftBillID)
                ...
                for (var i = 0; i < reqObj.baseTransactions.length; i++) {
                    ...
                    await Promise.all([objValidateInvs.checkLRExists(reqObj.baseTransactions[i]["baseTransactionNumber"], reqObj.baseTransactions[i]["glNumber"], "", "", draftBillId), objValidateInvs.checkAmt(reqObj.baseTransactions[i]["amount"], '-LR Amount cannot be blank,0 or less.'), objValidateInvs.checkGLMaster(reqObj.baseTransactions[i]["glNumber"])]).then(function (values) {
                        values.forEach(function (item, index, array) {
            ...
            _.forEach(reqObj.baseTransactions, (fl, key) => {
                ...
                //let baseTransDate = moment((fl.baseTransactionDate).toString()).format('YYYY-MM-DD');
                ...
                console.log(fl.baseTransactionNumber + '/' + fl.glNumber + ' Lr in SAP-->', lrFound) //+ '/' + baseTransDate + '/' + fl.baseTransactionType 
                ...
                if (lrFound == undefined)
                    ...
                    if (lrFound.length == 1) {
                        ...
                        valMsg += "LR(" + fl.baseTransactionNumber + "/" + fl.glNumber + ") multiple data found";
                    else
                        valMsg += "LR(" + fl.baseTransactionNumber + "/" + fl.glNumber + ") not found";
                }
        ...
        if (reqObj.event == "CRE") {
            ...
            if (reqObj.baseTransactions != null && reqObj.baseTransactions.length > 0) {
